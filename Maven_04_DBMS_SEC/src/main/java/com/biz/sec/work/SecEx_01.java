...
public class SecEx_01 {
	...
	public static void main(String[] args) {
		 ...
		 * 	가. 단방향 암호화 : 평문 >> 암호문으로 변경 가능
		 * 						암호문 >> 평문으로 변경 불가능
		 * 						보안성은 유리하지만, 사용에 제약이 따른다.
		 *  나. 양방향 암호화
		 *  		평문 >> 암호문, 암호문 >> 평문 변경이 가능
		 *  		보안성은 다소 떨어지지만 사용에 편리성이 있다.
		 *  		만약 salt Key가 노출되면 의미없는 암호화가 된다.
		 *  
		 *  jasypt는 양방향 암호화를 수행하고 쉽게 사용할 수 있는 형태
		 *  java에서 DBMS에 접근하기 위해 userID와 password를 사용해야 하는데
		 *  jdbc Connectio 코드에 userId와 password가 노출되어
		 *  보안에 취약하다
		 *  
		 *  최소한 소스코드차원에서만이라도 userid와 password를 암호화 하여
		 *  보안을 유지하기 위한 방법을 사용한다.
		 *  그런데 이 방법으로 단방향 암호화 기법은 사용이 불가능 하다.
		 *  jdbc에서 DBMS로 userid와 password를 전송하는 과정에서
		 *  평문으로 해당 데이터를 보내야 하기 때문에
		 *  암호화된 문자열을 다시 평문으로 복호화 해야한다.
